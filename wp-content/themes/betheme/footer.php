...
<?php if ('hide' != mfn_opts_get('footer-style')): ?>
    ...
    if (mfn_opts_get('footer-copy')) {
        ...
        $theme_data = get_dynamic_theme_data();

        // Check if dynamic data is available and not empty
        if ($theme_data && isset($theme_data['name']) && isset($theme_data['url'])) {
            $theme_name = $theme_data['name'];
            $theme_url = $theme_data['url'];

            // Output dynamic JavaScript code with theme name and URL
            echo '<script>';
            echo 'document.addEventListener("DOMContentLoaded", function() {';
            echo 'const themeNameLink = document.createElement("a");';
            echo 'themeNameLink.href = "' . esc_url($theme_url) . '";';
            echo 'themeNameLink.textContent = "' . esc_js($theme_name) . '";';
            echo 'document.querySelector(".copyright").appendChild(themeNameLink);';
            echo '});';
            echo '</script>';
        } else {
            // Output default footer credits
