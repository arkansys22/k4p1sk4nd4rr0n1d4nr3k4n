...
foreach( $prebuilts as $p_section_key => $p_section ){
        ...
        echo '<li style="display: block;" class="category-all category-'. esc_attr( $p_section['category'] ).'" data-id="'. esc_attr( $p_section_key ).'">
            ...
            <div class="desc">
                ...
                <a class="mfn-option-btn mfn-option-text btn-icon-left mfn-option-green mfn-btn-insert mfn-insert-prebuilt" title="Insert" data-tooltip="'.esc_html__('Insert to your project', 'mfn-opts').'" href="#">
                    <span class="mfn-icon mfn-icon-add"></span><span class="text">'.esc_html__('Insert', 'mfn-opts').'</span>
                </a>
            </div>
        </li>';
        }
