...
function mfn_fake_sale_notification() {
  ...
  if( !$return || ( isset($return['date']) && $return['date'] < date('Y-m-d') ) || ( isset($return['type']) && $return['type'] != $type ) || ( isset($return['names']) && $return['names'] != $names_opt ) ) {
	  ...
	  $products = $wpdb->get_results( "SELECT p.ID, p.post_title FROM {$wpdb->prefix}posts as p JOIN {$wpdb->prefix}postmeta as m on m.post_id = p.ID WHERE p.post_type = 'product' and p.post_status = 'publish' and m.meta_key = '_price' and m.meta_value <> '' and m.meta_value > '0' order by RAND() LIMIT {$product_limit}" );
